...
export default class MyPlugin extends Plugin {
  ...
  async sendMessage(text: string) {
    if (!this.settings.token || !this.settings.chatId) {
      return
    }

    const url = `https://api.telegram.org/bot${this.settings.token.trim()}/sendMessage`
    const body = {
    ...
    try {
      ...
      console.error('Failed to send message via Telegram API:', e)
    }
  }
  ...
  async messagesPooling() {
    ...
    dates.forEach(item => {
      ...
      if (found && found.groups && found.groups.body) {
        ...
        if (!check) {
          if (body && body.groups) {
            const dateString = body.groups.date + ' ' + body.groups.time
